...
The gates that run on every pull request: `truewire check` (the spec against its own rules and every recording against its schema), `truewire standards`, the generated code being current in all three backends, pytest, pyright, ruff, and the TypeScript and Rust clients' own builds and replays — 70 Python tests, 12 TypeScript, 9 Rust, all against `truewire mock`. Every code block on every one of these pages is compiled against its own package.
